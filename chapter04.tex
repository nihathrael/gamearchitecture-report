...
complete list and details. These XML components are transferred to Javascript classes, which are inherited from C++
component instances when loaded. A basic UML architecture diagram for the Javascript classes is given in \figref{TODO}. 

\paragraph{Datadriven?}
\AD{} is completely datadriven. All information needed to construct a unit is saved in the basic XML format.
Interestingly many components allow being extended by using Lua\footnote{http://www.lua.org/} scripts. This way not only
the unit's values, but also their behaviour is saved outside the basic engine code, decoupling the engine completely
from the game logic. 
