...
Two methodes were used when converting the SQLite data to YAML data: 
\begin{itemize}
    \item manual conversian
...
helper scipt was implemented in Python, which automatically converted the entire list of buildings into YAML building
definitions. This has the added benifit of avoiding syntax errors, as I let the YAML parser write the file instead of
...
To compare the old inheritance based class hierarchie, we compare the old UML tree given in \figref{fig:settleruml} with
